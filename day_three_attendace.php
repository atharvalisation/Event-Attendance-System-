    ...
    if (isset($_POST['faculty_id']) && is_array($_POST['faculty_id'])) {
        foreach ($_POST['faculty_id'] as $id => $attendance) {
            ...
            if ($facultyResult->num_rows > 0) {
                ...
                $checkSql = "SELECT * FROM day_three WHERE faculty_id = '$id'";
                $checkResult = $conn->query($checkSql);

                if ($checkResult->num_rows > 0) {
                    // Update attendance if record exists
                    $updateSql = "UPDATE day_three SET attendance='$attendance', faculty_name='$faculty_name' WHERE faculty_id='$id'";
                    $conn->query($updateSql);
                } else {
                    // Insert new record if record doesn't exist
                    $insertSql = "INSERT INTO day_three (faculty_id, attendance, faculty_name) VALUES ('$id', '$attendance', '$faculty_name')";
...
$sql = "SELECT f.id, f.name, f.mobile_number, d.attendance, d.faculty_name
        ...
        LEFT JOIN day_three d ON f.id = d.faculty_id";
...
<head>
    ...
    <title>Day Three Attendance</title>
    <style>
        body {
...
<body>
    ...
    <h2>Mark Attendance for Day Three</h2>
